...
# (o servidor nunca compila e nunca roda build/testes; roda node em dist/):
#   - backend lobby+game compilados para JS em dist/ (sem tsx em runtime)
#   - packages config/shared/engine compilados para JS em dist/ (main/exports)
#   - node_modules de produção (raiz + db; tsx/esbuild ficam de fora)
...
# ── 2. Workspaces: packages + backends (manifests + fonte TS + tsconfig) ────
log "copiando packages e backends"
mkdir -p "$STAGING/packages"
cp packages/tsconfig.json "$STAGING/packages/tsconfig.json"
for pkg in config shared engine; do
  mkdir -p "$STAGING/packages/$pkg"
  cp "packages/$pkg/package.json" "$STAGING/packages/$pkg/"
  cp "packages/$pkg/tsconfig.json" "packages/$pkg/tsconfig.build.json" "$STAGING/packages/$pkg/"
...
for srv in lobby-server game-server; do
  ...
  cp "backend/$srv/tsconfig.json" "backend/$srv/tsconfig.build.json" "$STAGING/backend/$srv/"
  cp -r "backend/$srv/src" "$STAGING/backend/$srv/src"
done

# ── 3. node_modules completo (dev+prod; precisa do typescript) + build dist ──
log "npm ci (raiz; dev+prod; pode demorar)"
( cd "$STAGING" && npm ci --no-audit --no-fund )

log "compilando packages (dist)"
for pkg in config shared engine; do
  ( cd "$STAGING" && npm run build --workspace="@flicker/$pkg" )
done

log "compilando backends (dist)"
for srv in lobby-server game-server; do
  ( cd "$STAGING" && npm run build --workspace="@flicker/$srv" )
done
...
# ── 5b. Release: main/exports → dist e poda de devDependencies ──────────────
# Reescreve SÓ os package.json do staging (config/shared/engine) para apontar
# para dist/, depois remove node_modules e reinstala somente produção (sem
# tsx/esbuild/typescript).
log "reescrevendo package.json para dist e podando devDependencies"
node "$REPO_ROOT/scripts/rewrite-release-package-json.mjs" "$STAGING"
( cd "$STAGING" && rm -rf node_modules && npm ci --omit=dev --no-audit --no-fund )
