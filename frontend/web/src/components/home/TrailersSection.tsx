...
function TrailerPlayer({ trailer }: { trailer: Trailer }) {
  ...
  if (!src) {
    media = (
      <TrailerCover titulo={titulo}>
        <span className="flex flex-col items-center gap-4 px-6 text-center">
          ...
          <span className="text-muted text-lg italic text-center">{trailers.mensagens.indisponivel}</span>
        </span>
      </TrailerCover>
    )
  } else if (failed) {
    media = (
      <div>
        ...
        <p role="status" className="mt-3 text-muted text-lg">{trailers.mensagens.falha}</p>
      </div>
    )
  } else if (shouldLoad) {
    media = (
      <div className="relative w-full aspect-video overflow-hidden rounded-none bg-black group">
        ...
        />
        {!ready && (
          <p role="status" className="absolute top-3 left-3 m-0 rounded-md bg-black/70 px-3 py-1 text-white text-lg">
  ...
  if (!src) {
    return (
      <div ref={containerRef} className="group">
        ...
        <p className="mt-3 text-muted text-lg leading-relaxed">{descricao}</p>
      </div>
    )
  }
  ...
  return (
    <div ref={containerRef}>
      ...
      {media}
      <p className="mt-3 text-muted text-lg leading-relaxed text-center">{descricao}</p>
    </div>
  )
}
