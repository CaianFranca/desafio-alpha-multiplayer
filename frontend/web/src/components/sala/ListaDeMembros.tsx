  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-sm tracking-[0.18em] uppercase text-white/80">Equipe</p>
        <span
      ...
      <ul className="flex flex-col gap-3" aria-label="Lista de Membros" aria-live="polite">
        {membrosOrdenados.map((membro) => {
          ...
          return (
            <li
              key={membro.id}
              className={`flex items-center gap-4 p-4 bg-[#1e1e1e] border ${membroEhAnfitriao ? 'border-l-2 border-l-[#c9a86a] border-y-white/30 border-r-white/30' : 'border-white/30'}`}
            >
              ...
              <div
                className={`w-6 h-6 border flex items-center justify-center shrink-0 ${membro.prontidao ? 'border-green-500 text-green-400' : 'border-white/30 text-white/80'}`}
                aria-label={
                  membroEhBot
