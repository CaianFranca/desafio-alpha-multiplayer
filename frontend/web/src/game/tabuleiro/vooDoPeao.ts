 ...
 * `PEAO_POSICIONADO` (Primeiro Turno) e `PEAO_PERMANECEU` pedem snap, sem voo
 * nem som — a issue cita só o evento de movimentação.
 *
...
/**
 * Deriva o voo base do `PEAO_MOVIDO` (único evento com voo): destino é a
 * célula do evento; origem é a célula da peça de `pecaIdDe` em `posicionadas`,
 * com fallback para a célula do peão no modelo anterior. Origem irresolúvel
 * (ou idêntica ao destino) → `null` = snap, sem voo nem som.
 * `PEAO_POSICIONADO`/`PEAO_PERMANECEU`/demais eventos → `null`.
 */
 ...
 * teria) — só o overlay voador aparece. Demais células/peões (ex.: Portão
 * com 4) seguem intactos. Sem voo, nada se suprime.
 */
...
export function vooPoseNoProgresso(
  ...
  const t = prenderEm01(progresso)
  const [ox, oy, oz] = mundoDoPeaoSobreACelula(origem)
  const [dx, dy, dz] = mundoDoPeaoSobreACelula(destino)
  // Pontas exatas (pixel-igual ao estático): `sin(pi) ≈ 1.2e-16`, não zero —
  // sem o guarda, o pouso herdaria um epsilon de ponto flutuante.
  const arco = t <= 0 || t >= 1 ? 0 : Math.sin(Math.PI * t) * VOO_ALTURA_MAX
