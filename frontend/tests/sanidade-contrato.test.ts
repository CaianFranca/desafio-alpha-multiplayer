...
describe('percepção de Sanidade e estados no cliente — tradução dos novos eventos (issue #174)', () => {
  ...
  it('aplicarSnapshot popula sanidade e estados (fonte autoritativa para late-join)', () => {
    ...
    expect(estado.jogadorPorId['j1']).toEqual({ apelido: 'Ana', cor: 'branco', sanidade: 3, emBaixaIluminacao: false, amedrontado: false, protegido: false, ordem: 0 })
    expect(estado.jogadorPorId['j2']).toEqual({ apelido: 'Bob', cor: 'vermelho', sanidade: 1, emBaixaIluminacao: true, amedrontado: false, protegido: false, ordem: 1 })
    expect(estado.jogadorPorId['j3']).toEqual({ apelido: 'Carol', cor: 'azul', sanidade: 0, emBaixaIluminacao: false, amedrontado: true, protegido: false, ordem: 2 })
