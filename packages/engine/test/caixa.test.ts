...
test('o fluxo legado de escolha de tipo consome a primeira peça do tipo da caixa', () => {
  // Estado com Recebimento pendente: encaixe direto da Peça Inicial seguido
  // das pendências do Recebimento montadas via literal (mesmo padrão dos
  // testes do ciclo do Peão — a geração pertence à camada da Partida).
  let base = aplicarComandoDeTabuleiro(estadoInicial(), {
    tipo: 'selecionar_peca',
    pecaId: 'inicial-1',
  });
  assert.equal(base.sucesso, true);
  if (!base.sucesso) return;
  base = aplicarComandoDeTabuleiro(base.estado, {
  ...
  const recebidaId = 'recebida-inicial-1-norte';
  ...
  const primeiraReta = estado.caixa.find((peca) => peca.tipo === 'reta');
  assert.ok(primeiraReta);

  const escolha = aplicarComandoDeTabuleiro(estado, {
  ...
  // A primeira reta da Caixa saiu dela e virou a Peça da Recebida.
  assert.deepEqual(
    escolha.eventos.filter(
      (evento) => evento.tipo === 'tipo_da_peca_recebida_escolhido',
    ),
    [
      {
        tipo: 'tipo_da_peca_recebida_escolhido',
        recebidaId,
        pecaId: primeiraReta.pecaId,
        tipoDaPeca: 'reta',
      },
    ],
  );
  assert.ok(
    !escolha.estado.caixa.some((peca) => peca.pecaId === primeiraReta.pecaId),
  );
  assert.equal(escolha.estado.caixa.length, 70);
});
